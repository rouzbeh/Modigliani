...
/* $Id: nt3d_abstract_vp_obj.h,v 1.1 2001/06/29 13:16:55 face Exp $
...
public:
    ...
    NTreturn Resize(NTsize newWidth, NTsize newHeight) {
        ResizeChild(newWidth,newHeight);
        return pVP->Resize(newWidth,newHeight);
    }
    void Redraw() {
        RedrawChild();
        pVP->Redraw();
    ...
    NT3D_bitmap_o View2Bitmap() {
        return (NT3D_bitmap_o) pVP->View2Bitmap();
    }
    NT3D_bitmap_o * View2BitmapPtr(NT3D_bitmap_o * bmpPtr) {
        return pVP->View2BitmapPtr(bmpPtr);
    }
    NTsize _width() {
        return pVP->_width();
    }
