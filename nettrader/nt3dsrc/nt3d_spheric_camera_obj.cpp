...
/* $Id: nt3d_spheric_camera_obj.cpp,v 1.2 2005/04/25 13:55:25 face Exp $ 
...
NT3D_spheric_camera_o::DrawChild()
{
	...
	for (NTsize llx=0; llx < aziSteps; llx++){
		for (NTsize lly=0; lly < altSteps; lly++){
			NTsize llxCounter = llx - floor(aziSteps/2.0);
			NTsize llyCounter = lly - floor(altSteps/2.0);
			glViewport(llx*deltaX,lly*deltaY, deltaX, deltaY);
			glMatrixMode(GL_PROJECTION);
			glLoadIdentity();
			gluPerspective(degY,degX/degY, 0.01, 100);
			// 2DO here we omit the orientation of the camera
			glMatrixMode(GL_MODELVIEW);
			glLoadIdentity();
			//glTranslatef( -pos.x, -pos.y, -pos.z);
			glRotatef(llxCounter*degX, 1.0, 0.0, 0.0);
			glRotatef(llyCounter*degY, 0.0, 1.0, 0.0);
			/* THIS CLASS IS A MESS ; AAF,25.02.2002
			 why  is the camera not a camera object ?
			 why do we also have universe_camera vp ?
			gluLookAt(0, 0, 0,
			          0, sin((-llx)*degX/360.0/M_PI/2.0),cos((-lly)*degY/360.0/M_PI/2.0),
			          0,1,0);
				  */
      			//glTranslatef( pos.x,pos.y,pos.z);
			DrawUniverse();
