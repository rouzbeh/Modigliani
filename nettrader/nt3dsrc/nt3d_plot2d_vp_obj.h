...
/* $Id: nt3d_plot2d_vp_obj.h,v 1.1 2001/06/29 13:16:55 face Exp $
...
protected:
    /* ***  Methods              ***/
    void InitChild() {};
    NTreturn ResizeChild(NTsize x, NTsize y) {
        return NT_SUCCESS;
    }
    void RedrawChild() {
    ...
    void NewXData(NTreal x) {
        ...
        else if (x > dataXMax) dataXMax = x;
    }
    void NewYData(NTreal y) {
        if (y < dataYMin) dataYMin = y;
        else if (y > dataYMax) dataYMax = y;
    }
    /* ***  Data                 ***/
    /**  */
