...
/* $Id: myelinaxonsimulator-v1.cpp,v 1.1 2005/04/25 15:13:17 face Exp $
	...
	*/
	...
	NT_config_file_parser_o oCfg("config.txt");
	...
	string outputFilename = oCfg.Value("simulation","outputFile");
...
if(false == swComputeELeak) {
   ...
   NTBP_custom_cylindrical_compartment_o compartment (10  /* muMeter */,10        /* muMeter */,1 /*muFarad/cm^2 */ ,100 /* ohm cm */ );
   NTreal areaPerCompartment = compartment._area ();
   compartment.Set_temperature (temperature /* in celsius */ );
   
   NTBP_membrane_current_o* tmpLeakPtr = new NTBP_hh_sga_leak_current_o(compartment._area(), ndGLeak, eLeak);
   ...
   for (lt = 0; lt < 50.0/timeStep; lt++)
   {
      compartment.Step(0);
      if (lt%50 == 0) {
	 naCurrent  = compartment.AttachedReversalPotential(1)*(compartment.AttachedConductance(1));
	 kCurrent   = compartment.AttachedReversalPotential(2)*(compartment.AttachedConductance(2));
	 cerr << "I_Na=" << naCurrent << " I_K=" << kCurrent << " E_Leak=" << -(naCurrent + kCurrent)/tmpLeakPtr->_conductance() << endl;
      }	
   }		
   ...
   if ( (eLeak > compartment.AttachedReversalPotential(1) )
	||
	(eLeak < compartment.AttachedReversalPotential(2) ) )
   {
      cout << "Eleak might be out of biological plausible range." << endl;
      cout << "Typically E_Na="<<compartment.AttachedReversalPotential(1) << " < Eleak="<< eLeak << " < E_K=" <<compartment.AttachedReversalPotential(2) << endl;
      cout << "Does a stable resting potential exist at all? Check by increasing iteratiosn of current balance equation." <<endl;
      cerr << "Eleak might be out of biological plausible range." << endl;
      cerr << "Typically E_Na="<<compartment.AttachedReversalPotential(1) << " < Eleak="<< eLeak << " < E_K=" <<compartment.AttachedReversalPotential(2) << endl;
      cerr << "Does a stable resting potential exist at all? Check by increasing iteratiosn of current balance equation." <<endl;
   }
}
     ...
     ofstream file(outputFilename.c_str(), ios::binary);
...
for(NTsize lnd = 0; lnd < numNd; lnd++)
{
	...
	new NTBP_custom_cylindrical_compartment_o(lengthNd /* muMeter */,
						   diameter	/* muMeter */,
						   ndCm/*muFarad/cm^2 */ ,
						   ndRa /* ohm cm */ );
	tmpPtr->Set_temperature(temperature /* in celsius */ );
	/* Leak current is number 0 */
	tmpPtr->AttachCurrent(new NTBP_hh_sga_leak_current_o(tmpPtr->_area(), ndGLeak, eLeak), NTBP_LEAK);
	/* Na current is number 1 */
	...
	tmpPtr->AttachCurrent(new NTBP_hh_sga_leak_current_o(tmpPtr->_area(), 0, eLeak), NTBP_LEAK);
	...
	NTBP_custom_cylindrical_compartment_o *tmpPtr =
	new NTBP_custom_cylindrical_compartment_o(lengthPnd /* muMeter */,
						   diameter	/* muMeter */,
						   pndCm/*muFarad/cm^2 */ ,
						   pndRa /* ohm cm */ );
	tmpPtr->Set_temperature(temperature /* in celsius */ );
	/* Leak current is number 0 */
	tmpPtr->AttachCurrent(new NTBP_hh_sga_leak_current_o(tmpPtr->_area(), pndGLeak, eLeak), NTBP_LEAK);
	/* Dummy zero leak current is number 1 */
	tmpPtr->AttachCurrent(new NTBP_hh_sga_leak_current_o(tmpPtr->_area(), 0.001, eLeak), NTBP_LEAK);
	/* K current is number 2 */
	tmpPtr->AttachCurrent(
		NTBP_create_k_channel_ptr(
			pndPotassiumModel, 
			pndPotassiumAlg, 
			pndPotassiumDensity /* mum^-2 */,
			pndPotassiumConductance /* pS */, 
	...
	if((numNd - 1) == lnd) { break;}
	
	/* Create an Internode compartment */
	for( NTsize lcomp = 0; lcomp < numIntComp; lcomp++){
	cerr << "Internode compartment "<<endl;
	NTBP_custom_cylindrical_compartment_o *tmpPtr =
	new NTBP_custom_cylindrical_compartment_o(lengthIntNd /* muMeter */,
						   diameter	/* muMeter */,
						   intCm/*muFarad/cm^2 */ ,
						   intRa /* ohm cm */ );
	tmpPtr->Set_temperature(temperature /* in celsius */ );
	/* Leak current is number 0 */
	tmpPtr->AttachCurrent(new NTBP_hh_sga_leak_current_o(tmpPtr->_area(), intGLeak, eLeak), NTBP_LEAK);
	/* Dummy zero leak current is number 1 */
	tmpPtr->AttachCurrent(new NTBP_hh_sga_leak_current_o(tmpPtr->_area(), 0.001, eLeak), NTBP_LEAK);
	/* K current is number 2 */
	tmpPtr->AttachCurrent(
	...
	NTsize numCompartments =(numNd-1)*(numNdComp+numPndComp+numIntComp+numPndComp)+numNdComp+numPndComp;
	cerr << "Total number of compartments(computed)" << numCompartments << endl;
	cerr << "Total number of compartments(in oModel)" << oModel._numCompartments() << endl;
      ...
      /* Graphics init */
       NT3D_plot2d_vec_vp_o plotXY(numCompartments);
          ...
          NT3D_plot2d_vec_vp_o plotChanNa(numCompartments);
           ...
           NT3D_glx_drv_o* drv2VP =  new  NT3D_glx_drv_o(500,200);
            drv2VP->SetWindowTitle("NaOpenChannelRatio-Compartment-Plot");
              if(NT_FAIL == plotChanNa.Connect(drv2VP)) exit(1);
        ...
        if(useVis > 0) {
          ...
          if(NT_FAIL == plotChanK.Connect(drv3VP)) exit(1);
    ...
    */
     ...
     for(NTsize lt = 0; lt < numIterations; lt++)
        ...
        timeVar = timeInMS;
 		  ...
 		  if(lt % sampN == 0)
 			...
 			file.write(reinterpret_cast < char *>(&timeVar),
 				    sizeof(float));
 	...
 	for(NTsize lc = 0; lc++; lc<numCompartments){
		if(NTisnan(voltVec[lc]))	{
 		cerr << "ERROR at t=" << timeVar <<" voltage in compartment "<<lc<<" is NaN." << endl;
 		exit(1);
 		} else if(voltVec[lc] > 200.0 /* mV */)	{
 		cerr << "ERROR at t=" << timeVar <<" voltage in compartment "<<lc<<" is "<<voltVec[lc]<<"." << endl;
 		exit(1);
 		}
 	}

	cerr << "t="<<timeInMS<<" mSec :"<< voltVec[1] << "\t" << voltVec[10] << endl;
