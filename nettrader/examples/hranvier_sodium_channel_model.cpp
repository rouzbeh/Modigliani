...
/* $Id: hranvier_sodium_channel_model.cpp,v 1.3 2003/06/20 13:25:55 face Exp $
* $Log: hranvier_sodium_channel_model.cpp,v $
...
main (int argc, char *argv[])
{
    ...
    for (NTreal vTarget=-20;vTarget<=130;vTarget+=10.0) {
        for (NTsize lt=0; lt<numTrials;lt++) {
            /* *********************************************************  Main loop ************************************************ */
            cerr << "MainLoop started" << endl;

            NTreal timeInMS = 0.0;
            NTBP_hranvier_sodium_multi_current_o current(100,10);
            current.SetSimulationMode (NTBP_SINGLECHANNEL);
