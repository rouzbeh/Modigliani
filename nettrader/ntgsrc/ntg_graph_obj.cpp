        ...
        nodeIdManager(1, NTG_MAX_NODES_IN_GRAPH)
{
    ...
    cerr << "NTG_graph_o - WARNING : The entire code of this class should be checked as changes where " <<
         " necessary to make it compile on gcc 3.0. especially the handling of NULL pointer on iterators should" <<
         " be removed and replaced by more STL-style algorithms. AAF, 26.2.2002. " << endl;
...
NTG_graph_o::NTG_graph_o(const NTG_graph_o & original)
        :
        nodeIdManager(original.nodeIdManager)
{
...
*/
NTid
NTG_graph_o::AddNode(const NTG_node_o & newNode)
{
    NTid tmp = nodeIdManager.IssueId();
    if (tmp < 1) {
        cerr << "NTG_graph_o::AddNode - Error : nodeIdManager returned error state."<< endl;
        return 0;
    }
...
*/
NTreturn
NTG_graph_o::Connect(NTid newSource, NTid newTarget)
{
    NTG_edge_o tmp;
...
*/
NTreturn
NTG_graph_o::Connect(NTid newSource, NTid newTarget, NTG_edge_o newEdge)
{
    NTbool error = false;
    if (false == InGraph(newSource) ) {
        cerr << "NTG_graph_o::Connect - Error : Not existing source node id="<<newSource<<" specified." << endl;
        error = true;
    }
    if (false == InGraph(newTarget) ) {
        cerr << "NTG_graph_o::Connect - Error : Not existing target node id="<<newTarget<<" specified." << endl;
        error = true;
    }
    if (true == error) return NT_NOT_EXISTING_NODE;
...
*/
NTreturn
NTG_graph_o::SymmetricConnect(NTid a, NTid b)
{
    NTG_edge_o tmp;
...
*/
NTreturn
NTG_graph_o::SymmetricConnect(NTid a, NTid b, const NTG_edge_o & newEdge)
{
    NTreturn tmp1 = Connect(a,b,newEdge);
    NTreturn tmp2 =	Connect(b,a,newEdge);

    if (NT_SUCCESS != tmp1) return tmp1;
    else if (NT_SUCCESS != tmp2) return tmp2;
    return NT_SUCCESS;
}
...
*/
NTreturn
NTG_graph_o::Disconnect(NTid newSource, NTid newTarget)
{

    NTbool conn = IsConnected(newSource, newTarget);
    if (false == conn) {
        cerr << "NTG_graph_o::Disconnect - Error : Attempts to disconnect non-existing edge ("<<newSource<<","<<newTarget<<").";
...
//	cerr << "NTG_graph_o::Disconnect - Talk: currently "<<E.size() <<" in list. Deleting..." << endl;
    ...
    cerr << "commented out line for compiling (gcc 3.0.1) does the code work" << endl;
//	if (  NULL == itE ) {
...
NTreturn NTG_graph_o::SymmetricDisconnect(NTid a, NTid b)
{
    ...
    if (returnVal1 != returnVal2) {
        if (NT_SUCCESS == returnVal1) return returnVal2;
        else if (NT_SUCCESS == returnVal2) return returnVal1;
        else {
            cerr <<"NTG_graph_o::SymmetricDisconnect - Local invalid state 0 occurred" << endl;
            return NT_FAIL;
        }
    }
    else if (returnVal1 == returnVal2) {
        if (NT_SUCCESS == returnVal1) return NT_SUCCESS;
        else return returnVal1;
    }
    else {
        cerr <<"NTG_graph_o::SymmetricDisconnect - Local invalid state 1 occurred" << endl;
        return NT_FAIL;
    }
}
...
bool NTG_graph_o::InGraph(NTid selNode) const
{
    ...
    for (itV = V.begin(); itV != V.end(); itV++) {
        if ( (*itV).first == selNode ) {
            if (true == isPresent) {
                error = true;
                cerr << "NTbool NTG_graph_o::InGraph - Error : Node id present multiple times in Vertex list." << endl;
                return false;
            } else {
                isPresent = true;
            }
    ...
    if (idManagerResponse != isPresent) {
        cerr << "NTbool NTG_graph_o::InGraph - Error : Mismatch, node id manager says " << idManagerResponse <<
             " while node list search results in " << isPresent << ".\n";
        return false;
    } else {
...
/** @short
   ...
   \bug
*/
NTG_NodeDistanceMatrix NTG_graph_o::NodeDistanceMatrix(const NTG_NodeDistanceMatrix & aMtr) const
{
    NT_ASSERT( ( (NTsize) aMtr.num_rows() == NumNodes() ) && ( (NTsize)aMtr.num_cols() ==NumNodes()) );
...
{
    NTG_NodeDistanceMatrix dMtr = NodeDistanceMatrix( NodeDistanceMatrixConditioning( AdjacencyMatrix() ) );
    NT_sequential_statistics_o oStats;

    NT_ASSERT (NTsize(dMtr.size()) == NTsize(NumNodes()*NumNodes()) ); /* simple test, 2DO improve */
    NTsize lr, lc;
    for (lr=0; lr < NumNodes(); lr++) {
        for (lc=0; lc < NumNodes(); lc++) {
...
NTbool NTG_graph_o::IsConnected(NTid source, NTid target) const
{
    if (InGraph(source) && InGraph(target)) {
        ...
        for (itE = E.begin(); itE != E.end(); itE++) {
            if (	((*itE).Source() == source) && ((*itE).Target() == target) ) return true;
        }
        return false;
    } else {
        cerr << "NTG_graph_o::IsConnected - Warning : Source or Target node id do not exist in graph. Returning 'false'.";
        return false;
    }
}
...
NTbool NTG_graph_o::IsSymmetricConnected(NTid a, NTid b) const
{
    ...
    if (InGraph(a) && InGraph(b)) {
        for (itE = E.begin(); itE != E.end(); itE++) {
            if (	((*itE).Source() == a) && ((*itE).Target() == b) ) oneway = true;
            else if (	((*itE).Source() == b) && ((*itE).Target() == a) ) otherway = true;
        }
        return (oneway && otherway);
    } else {
        cerr << "NTG_graph_o::Connected - Warning : Source or Target node in graph. Returning 'false'.";
        return false;
    }
}
...
*/
NTreturn
NTG_graph_o::DeleteNode(NTid delNode)
{
    if (InGraph(delNode) != true) return NT_NOT_EXISTING_NODE;
...
//	if (NULL == itV) cerr << "NTG_graph_o::DeleteNode - Error : No iterator returned." << endl;
    ...
    if (InGraph(delNode) == false) {
        ...
        UpdateStatistics();
        cerr << "NTG_graph_o::DeleteNode - Error : InGraph() returned true, but node was not found in vertex list." << endl;
        return NT_NOT_EXISTING_NODE;
    }
...
{
    NTG_AdjacencyMatrix aMtr(nodeIdManager.LargestIssuedId(),nodeIdManager.LargestIssuedId(),false);
...
NTG_EdgeContainer NTG_graph_o::ConvergentConnections(NTid id) const
{
    ...
    for (itE = E.begin(); itE != E.end(); itE++) {
        if ( (*itE).Target() == id ) tmpE.push_back(*itE);
    }
...
NTG_EdgeContainer NTG_graph_o::DivergentConnections(NTid id) const
{
    ...
    for (itE = E.begin(); itE != E.end(); itE++) {
        if ( (*itE).Source() == id ) tmpE.push_back(*itE);
    }
...
NTid NTG_graph_o::RandomNodeId() const
{
    ...
    if (NumNodes() == 1) return (*(V.begin())).first;
    else {
        do {
...
NTreturn NTG_graph_o::DisconnectNode(NTid node)
{
    ...
    if (di != NT_SUCCESS) return di;
    else if ( co != NT_SUCCESS) return co;
...
//	if (false == InGraph(id)) return NULL;
    ...
    for (itV = V.begin(); itV != V.end(); itV++) {
        if ((*itV).first == id) return itV;
    }
    cerr << "NTG_graph_o::NodeIterator - Error : Control flow should not have reached here." << endl;
//	return NULL;
}
...
/** @short     Return iterator to corresponding node id.
   ...
   \bug        unknown
*/
NTG_EdgeContainer::iterator NTG_graph_o::EdgeIterator(NTid source, NTid target)
{
//	if (IsConnected(source,target) == false) return NULL;
    NTG_EdgeContainer::iterator itE;
    for (itE = E.begin(); itE != E.end(); itE++) {
        if (	((*itE).Source() == source) && ((*itE).Target() == target) ) return itE;
    }
    cerr << "NTG_graph_o::EdgeIterator - Error : Control flow should not have reached here." << endl;
//	return NULL;
}
...
void NTG_graph_o::UpdateStatistics() {
    numNodes = V.size();
    numEdges = E.size();
    if (numNodes > 1) rndNodeGen = NT_uniform_rnd_dist_o(1, nodeIdManager.LargestIssuedId());
}
