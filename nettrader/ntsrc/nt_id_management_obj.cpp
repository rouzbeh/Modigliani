...
// cerr << "NT_id_management_o::NT_id_management_o(NTid smallestId, NTid largestId)" << endl;
    ...
    for (it = usedIdList.begin(); it != usedIdList.end(); it++) (*it) = (lowestId-1);
}
 ...
 */
NTbool
NT_id_management_o::IsTaken(NTid id) const
{
    /*set<NTid> tmp;
    ...
    if ((id < lowestId) || (id > highestId)) {
        cerr << "NT_id_management_o::IsTaken - Warning : Query id="
             <<id<<" out of id manager range ["<<lowestId<<","
             <<highestId<<"]. Will return 'false'."
             << endl;
        return NT_FALSE;
    }
 ...
 */
NTid
NT_id_management_o::LargestIssuedId()
{
 ...
 */
NTreturn
NT_id_management_o::FreeId(NTid id)
{
