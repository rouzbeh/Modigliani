...
vector <NTreal>
NT_make_vector_func( NTsize dim )
{
...
vector <NTreal> NT_binomial_rnd_dist_o::logGammaTable = NT_make_vector_func(NT_LOGGAMMATABLEMAX);
        ...
        NT_rnd_dist_o()
{
    ...
    if (false == init) {
        cerr << "NT_binomial_rnd_dist_o::NT_binomial_rnd_dist_o - Talk : This may take a while, populating LogGamma(0.."<<NT_LOGGAMMATABLEMAX-1<<") tables." << endl;
...
/* ***      COPY AND ASSIGNMENT	***/
NT_binomial_rnd_dist_o::NT_binomial_rnd_dist_o(const NT_binomial_rnd_dist_o & original)
{
 ...
 */
inline NTreal
NT_binomial_rnd_dist_o::RndVal() const
{
//	return Binomial(p,n);
    ...
    */
    ...
    static float al,alv,amaxp,c,f,f1,f2,ffm,fm,g,pp,p1,p2,p3,p4,q,qn,r,u,v,w,w2,x,x1,
...
S130:
    ...
    */
    ...
    f2 = f1*f1;
    w2 = w*w;
    if (alv <= xm*log(f1/x1)+(n-m+0.5)*log(z/w)+(ix-m)*log(w*pp/(x1*q))+(13860.0-
...
NTreal NT_binomial_rnd_dist_o::Binomial(float pp, int n) const
{
    ...
    if ( n < 25) {
        bnl = 0.0;
        for (j=1;j<=n;j++)	if (uniformMT() < p) ++bnl;
    } else if (am < 1.0) {
        g = exp(-am);
        t = 1.0;
        for (j=0; j<=n;j++) {
