...
*/
template<class T> NT_link_list_o<T>::NT_link_list_o(const T& i, NT_link_list_o<T>* n) :
        _info(i),
        _next(n)
{}
...
template<class T> const NT_list_o<T>& NT_list_o<T>::operator=(const NT_list_o<T>& b)
{
    if (this != &b)
    {
        free();
...
*/
template<class T> NT_iterator_list_o<T>::NT_iterator_list_o(const NT_list_o<T>& l)
        :  _list(l),
        _pre(0)
{}
