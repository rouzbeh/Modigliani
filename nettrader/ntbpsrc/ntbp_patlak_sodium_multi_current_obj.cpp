...
/* $Id: ntbp_patlak_sodium_multi_current_obj.cpp,v 1.1.1.1 2004/12/16 01:38:36 face Exp $ 
...
:
			NTBP_multi_current_o(
					115 /* in mV */, //2DO IS THIS THE RIGHT APPROACH VALUES ARE NOT FOR THESE VOLTAGES
					newDensity /* channels per mu^2 */, newArea /* in mu^2 */,
					newConductivity /* in mS per channel  */, -70 /* mV */
			) {
	// general
	...
	UpdateNumChannels();
	channelsPtr = new NTBP_ion_channels_o(_numChannels(), 7);
...
NTBP_patlak_sodium_multi_current_o::operator=(
		const NTBP_patlak_sodium_multi_current_o & right) {
	if (this == &right)
...
/** @short       
 ...
 \bug        unknown
 */
void NTBP_patlak_sodium_multi_current_o::ComputeRateConstants(NTreal vM /* in mV */) {
...
inline NTreturn NTBP_patlak_sodium_multi_current_o::StepCurrent() {
	...
	default:
		cerr
				<< "NTBP_patlak_sodium_multi_current_o::StepCurrent - ERROR : Unsupported simulation mode."
				<< endl;
...
inline NTreal NTBP_patlak_sodium_multi_current_o::ComputeConductance() {
	...
	default:
		cerr
				<< "NTBP_patlak_sodium_multi_current_o::ComputeConductance - ERROR : Unsupported simulation mode for ComputeConductance."
				<< endl;
		return 0;
	}
}
...
/**  */
/** No descriptions */
inline NTreal NTBP_patlak_sodium_multi_current_o::OpenChannels() const {
	switch (_simulationMode()) {
	case NTBP_BINOMIALPOPULATION:
	case NTBP_SINGLECHANNEL:
	...
	default:
		cerr
				<< "NTBP_patlak_sodium_multi_current_o::OpenChannels - ERROR : Unsupported simulation mode for OpenChannels."
				<< endl;
		return 0;
	}
}

inline NTreal NTBP_patlak_sodium_multi_current_o::ClosedChannels() const {
	switch (_simulationMode()) {
	case NTBP_BINOMIALPOPULATION:
	case NTBP_SINGLECHANNEL:
