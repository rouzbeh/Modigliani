...
NTBP_colbert_axonal_sodium_multi_current_o::operator=(
...
inline NTreturn NTBP_colbert_axonal_sodium_multi_current_o::StepCurrent() {
	...
	default:
		cerr
				<< "NTBP_colbert_axonal_sodium_multi_current_o::StepCurrent - ERROR : Unsupported simulation mode."
				<< endl;
...
inline void NTBP_colbert_axonal_sodium_multi_current_o::ComputeRateConstants() {

	NTreal vM = -100;
	NTreal temp = _temperature();
	NTreal deltaT = _timeStep();
	NTreal q10FactorM = NTBP_TemperatureRateRelation(temp, baseTemp /* C */, q10m);
	NTreal q10FactorH = NTBP_TemperatureRateRelation(temp, baseTemp /* C */, q10h);
	for (NTsize i = 0; i < 3000; i++) {
		...
		channelsPtr->setTransactionProbability(i, 4, 5, channelsPtr->getTransactionProbability(i, 0, 1));
		channelsPtr->setTransactionProbability(i, 5, 6, channelsPtr->getTransactionProbability(i, 1, 2));
		channelsPtr->setTransactionProbability(i, 6, 7, channelsPtr->getTransactionProbability(i, 2, 3));
		...
		channelsPtr->setTransactionProbability(i, 3, 2, channelsPtr->getTransactionProbability(i, 7, 6));
		channelsPtr->setTransactionProbability(i, 2, 1, channelsPtr->getTransactionProbability(i, 6, 5));
		channelsPtr->setTransactionProbability(i, 1, 0, channelsPtr->getTransactionProbability(i, 5, 4));
...
/**  */
/** No descriptions */
inline NTreal NTBP_colbert_axonal_sodium_multi_current_o::OpenChannels() const {
	switch (_simulationMode()) {
	case NTBP_BINOMIALPOPULATION:
	case NTBP_GILLESPIE:
	case NTBP_SINGLECHANNEL:
	...
	default:
		cerr
				<< "NTBP_colbert_axonal_sodium_multi_current_o::OpenChannels - ERROR : Unsupported simulation mode for OpenChannels."
				<< endl;
		return 0;
	}
}
...
inline NTreal NTBP_colbert_axonal_sodium_multi_current_o::ComputeConductance() {
	...
	default:
		cerr
				<< "NTBP_colbert_axonal_sodium_multi_current_o::ComputeConductance - ERROR : Unsupported simulation mode for ComputeConductance."
				<< endl;
		return 0;
	}
}

inline NTreal NTBP_colbert_axonal_sodium_multi_current_o::ComputeChannelStateTimeConstant() const {
