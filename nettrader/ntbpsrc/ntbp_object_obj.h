...
/* $Id: ntbp_object_obj.h,v 1.4 2003/01/17 16:53:13 face Exp $ /home/face/PRJ/NetTrader/src/ntbpsrc/ntbp_membrane_current_obj.cpp
...
public:
    ...
    /** @short  update of explicitly time dependent variables using a deltaT = timeStep, NO update
       ...
       \bug        unknown
    */
    NTreal UpdateTimeStep(NTreal newSuggestedTimeStep) { /* if (suggestedTimeStep > newSuggestedTimeStep) */
        suggestedTimeStep = newSuggestedTimeStep;
        return newSuggestedTimeStep;
    }
    virtual NTreturn Step() {
        cerr << "NTBP_object_o:Step() - Warning : Non-overriden base method () called by derived class." << endl;
