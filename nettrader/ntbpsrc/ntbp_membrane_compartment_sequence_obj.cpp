...
/* ***      COPY AND ASSIGNMENT	***/ 
NTBP_membrane_compartment_sequence_o::NTBP_membrane_compartment_sequence_o(const NTBP_membrane_compartment_sequence_o & original)
{
...
NTBP_membrane_compartment_sequence_o::operator= (const NTBP_membrane_compartment_sequence_o & right)
{
...
*/
NTreturn 
NTBP_membrane_compartment_sequence_o::PushBack(NTBP_cylindrical_compartment_o * compartPtr )
{
	...
	if (compartmentVec.size() != numCompartments) return NT_FAIL;
	return NT_SUCCESS;
}
...
*/
NTreturn
NTBP_membrane_compartment_sequence_o::Step() 
{
  //	cerr << "NTBP_membrane_compartment_sequence_o::Step()" << endl;
	if( true != initialised ){
		cerr << "NTBP_membrane_compartment_sequence_o::Step() - Warning : Called method with  Init() beeing called after instantiation or AddCompartment. Calling Init() now" << endl;
		if (Init() != NT_SUCCESS) {
			cerr << "NTBP_membrane_compartment_sequence_o::Step() - Error : Call to Init failed (No compartments present ?)." << endl;
			return NT_FAIL;
			}
	}
	...
	for (ll = 0; ll < numCompartments; ll++) {
		...
		omega = 1e-3 /* mV/muV */ * (_timeStep() / compartmentVec[ll]->_compartmentMembraneCapacitance())
...
*/
NTreturn
NTBP_membrane_compartment_sequence_o::Init()
{
	if (compartmentVec.size() <= 0) {
		cerr << "NTBPCompartmentMembraneNetCurrent()_membrane_compartment_sequence_o::Init - ERROR : No compartments present." << endl;
		initialised = false;
		return NT_FAIL;
	}
	...
	sigma = (0.1/2.0)*(deltaT/deltaXX)*(radius/axoplasmicR)*(compartmentVec[0]->_area()/compartmentVec[0]->CompartmentMembraneCapacitance());
	...
	for (ll = 1; ll < numCompartments-1; ll++) {
			...
			/* sigma is unit-free  (scaled by 0.1)     [sigma] = mSec muM / (muM^2 Ohm cm) * (muM^2 / muF) */
			sigma = (0.1/2.0)*(deltaT/deltaXX)*(radius/axoplasmicR)*(compartmentVec[ll]->_area()/compartmentVec[ll]->CompartmentMembraneCapacitance());			
			lVec[ll] = -sigma;
	...
	/* LAST COMPARTMENT */
	deltaXX = compartmentVec[numCompartments-1]->_length()  * compartmentVec[numCompartments-1]->_length();
	/* sigma is unit-free  (scaled by 0.1)     [sigma] = mSec muM / (muM^2 Ohm cm) * (muM^2 / muF) */
	sigma = (0.1/2.0)*(deltaT/deltaXX)*(radius/axoplasmicR)*(compartmentVec[numCompartments-1]->_area()/compartmentVec[numCompartments-1]->CompartmentMembraneCapacitance());			
...
vector <NTreal>
NTBP_membrane_compartment_sequence_o::OpenChannels (NTsize currIndex) const
{
...
vector <NTreal>
NTBP_membrane_compartment_sequence_o::NumChannels (NTsize currIndex) const
{
	vector <NTreal> tmp(_numCompartments());
	for (NTsize ll=0; ll< _numCompartments(); ll++){
...
vector <NTreal>
NTBP_membrane_compartment_sequence_o::OpenChannelsRatio (NTsize currIndex) const
{
	vector <NTreal> tmp(_numCompartments());
	for (NTsize ll=0; ll< _numCompartments(); ll++){
...
NTreturn
NTBP_membrane_compartment_sequence_o::WriteMembranePotential( ofstream & file )
{
...
/** @short  Write compartment currents into a binary file
	...
	\bug        unknown  */
NTreturn
NTBP_membrane_compartment_sequence_o::WriteCurrent( ofstream & file, NTsize index )
{
	float data[_numCompartments()];
	if (0 == index) {
		for (NTsize ll=0; ll< _numCompartments(); ll++){
...
NTreturn
NTBP_membrane_compartment_sequence_o::WriteOpenChannelsRatio( ofstream & file, NTsize index )
{
	float data[_numCompartments()];
...
/** @short  Give compartment currents
	...
	\bug        unknown  */
vector <NTreal>
NTBP_membrane_compartment_sequence_o::GiveCurrent(NTsize index)
{
...
/** @short
	...
	\bug        unknown  */
NTreturn
NTBP_membrane_compartment_sequence_o::InjectCurrent(NTreal current /* in nA */, NTsize compartmentId)
{
	if ((compartmentId < 1) || (compartmentId > _numCompartments())) return NT_PARAM_OUT_OF_RANGE;
...
vector <NTreal>
NTBP_membrane_compartment_sequence_o::ZadorPearlmutterSolveTriDiag(vector <NTreal> lNewVec,
										vector <NTreal> dNewVec,
										vector <NTreal> uNewVec,
										vector <NTreal> rNewVec) const
{
	...
	NTint j = 0;
	for(j = n-2; j > 0; j--){
		Kd[j] = 1/BJd[j] + (Kd[j+1]*lNewVec[j]*uNewVec[j])/(BJd[j]*BJd[j]);
		Ku[j] = -Kd[j+1]*uNewVec[j]/BJd[j];
		Kl[j] = -Kd[j+1]*lNewVec[j]/BJd[j];
	}

	for(ll = 1; ll < n-1; ll++){
		vNewVec[ll] = Kl[ll]*vNewVec[ll-1] + Kd[ll]*rNewVec[ll] + Ku[ll]*vNewVec[ll+1];
	}	
...
vector <NTreal>
NTBP_membrane_compartment_sequence_o::MascagniSolveTriDiag(vector <NTreal> lNewVec,
										vector <NTreal> dNewVec,
										vector <NTreal> uNewVec,
										vector <NTreal> rNewVec) const
{
	...
	for (i = 1; i < m-1; i++){
		...
		cout << i << " " << lNewVec[i] << " " << dNewVec[i] << " "<<uNewVec[i]<< " " << vNewVec[i] << " " << rNewVec[i] << endl;
	}
	dNewVec[m-1] = dNewVec[m-1] - lNewVec[m-1] * vNewVec[m-1-1];
	rNewVec[m-1] = (rNewVec[m-1] - lNewVec[m-1] * vNewVec[m-1-1])/dNewVec[m-1];
	uNewVec[m-1] = 0;

	/* backward substitution */
	vNewVec[m-1] = rNewVec[m-1];
	NTint ll = 0; // INT as NTsize cannot be compared to a negative number
	for	(ll = m-2; ll > -1; ll--) {vNewVec[ll] = rNewVec[ll] - uNewVec[ll] * vNewVec[ll+1];
		cout << ll << " " << lNewVec[ll] << " " << dNewVec[ll] << " "<<uNewVec[ll]<< " " << vNewVec[ll] << " " << rNewVec[ll] << endl;	}
	return vNewVec;
}
...
{
	cerr <<"NTBP_membrane_compartment_sequence_o::CompartmentSequenceChannelStateTimeConstant()" << endl;
	NTreal sum = 0.0;
	for (NTsize ll = 0; ll < numCompartments; ll++) {
...
/**  */
vector <NTreal> NTBP_membrane_compartment_sequence_o::NumericalRecipesSolveTriDiag(const vector <NTreal> & lNewVec, const vector <NTreal> & dNewVec, const vector <NTreal> & uNewVec, const vector <NTreal> & rNewVec) const
{
	...
	for( j=1; j < n; j++){
		...
		if (0 == bet) cerr <<"NTBP_membrane_compartment_sequence_o::NumericalRecipesSolveTriDiag - solver failed, zero pivot element ? is the matrix diagonally domininat ?)." << endl;
