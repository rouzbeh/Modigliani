...
/* ***      COPY AND ASSIGNMENT	***/
NTBP_migliore_sodium_multi_current_o::NTBP_migliore_sodium_multi_current_o(
		const NTBP_migliore_sodium_multi_current_o & original)
:
...
NTBP_migliore_sodium_multi_current_o::operator=(
		const NTBP_migliore_sodium_multi_current_o & right) {
	if (this == &right)
...
return NT_SUCCESS;
	...
	default:
		cerr
				<< "NTBP_migliore_sodium_multi_current_o::StepCurrent - ERROR : Unsupported simulation mode."
				<< endl;
...
inline void NTBP_migliore_sodium_multi_current_o::ComputeRateConstants() {
	...
	vM += 0.1;
			if ((vM < -20) || (vM > 130.0)) {
				q10Factor = NTBP_TemperatureRateRelation(temp, baseTemp /* C */,
						q10m);
				alphaM = q10Factor * AlphaM(vM);
				betaM = q10Factor * BetaM(vM);
...
/**  */
/** No descriptions */
inline NTreal NTBP_migliore_sodium_multi_current_o::OpenChannels() const {
	switch (_simulationMode()) {
	case NTBP_BINOMIALPOPULATION:
	case NTBP_GILLESPIE:
	case NTBP_SINGLECHANNEL:
	...
	default:
		cerr
				<< "NTBP_migliore_sodium_multi_current_o::OpenChannels - ERROR : Unsupported simulation mode for OpenChannels."
				<< endl;
		return 0;
	}
}
...
inline NTreal NTBP_migliore_sodium_multi_current_o::ComputeConductance() {
	...
	case NTBP_NOISYMEAN: {
		...
		return Set_conductance(
				_maxConductivity() /* mS/cm^2 */* mean * _area()
						/* muMeter^2 */* 1.0e-8 /* cm^2/muMeter^2 */);
		break;
	}
	default:
		cerr
				<< "NTBP_migliore_sodium_multi_current_o::ComputeConductance - ERROR : Unsupported simulation mode for ComputeConductance."
				<< endl;
		return 0;
	}
}

inline NTreal NTBP_migliore_sodium_multi_current_o::ComputeChannelStateTimeConstant() const {
