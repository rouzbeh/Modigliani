...
NTBP_multi_sodium_steadystate_timeconstant_current_o(											NTreal area /* in muMeter^2 */, 
...
NTBP_multi_sodium_steadystate_timeconstant_current_o(const NTBP_multi_sodium_steadystate_timeconstant_current_o & original);
const NTBP_multi_sodium_steadystate_timeconstant_current_o & operator= (const NTBP_multi_sodium_steadystate_timeconstant_current_o & right);
virtual ~NTBP_multi_sodium_steadystate_timeconstant_current_o();
/* ***  Methods              ***/  
NTreal AlphaM(NTreal vM /* in mV */) {vM+=_vBase(); return mInfty(vM)/tauM(vM); }
NTreal BetaM(NTreal vM /* in mV */) {vM+=_vBase(); return (1.0-mInfty(vM))/tauM(vM); }
NTreal AlphaH(NTreal vM /* in mV */) {vM+=_vBase(); return hInfty(vM)/tauH(vM); }
NTreal BetaH(NTreal vM /* in mV */) {vM+=_vBase(); return (1.0-hInfty(vM))/tauH(vM); }
...
inline NTreal Fermi(NTreal x, NTreal offset, NTreal temp){ return  1.0/(1.0 + exp( -(x-offset)/temp ) ); }
...
inline NTreal mInfty(NTreal vM) { return Fermi(vM,vMOffset,bM); }
inline NTreal tauM(NTreal vM) { return cMT;}
inline NTreal hInfty(NTreal vM) { return Fermi(vM,vHOffset,bH); }
