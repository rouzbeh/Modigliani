...
/* $Id: ntbp_tridiag_solver_obj.cpp,v 1.1 2001/10/03 14:37:52 face Exp $
...
/* ***      COPY AND ASSIGNMENT	***/
NTBP_tridiag_solver_o::NTBP_tridiag_solver_o(const NTBP_tridiag_solver_o & original)
{
 ...
 */
vector <NTreal>
NTBP_tridiag_solver_o::Solve( vector <NTreal> dVec,  vector <NTreal> vVec, vector <NTreal> rVec)
{
    cerr << "OBSOLETE code called";
    vector <NTreal> lVec;
    vector <NTreal> uVec;
    NT_ASSERT( true == initialised);
    NT_ASSERT( lVec.size() == dVec.size() == uVec.size() == vVec.size() == rVec.size() == numCompartments);
...
/**  */
NTreturn
NTBP_tridiag_solver_o::Resize(NTsize numNewCompartments)
{
    NT_ASSERT(numNewCompartments > 0 );
    numCompartments = numNewCompartments;
    return NT_SUCCESS;
