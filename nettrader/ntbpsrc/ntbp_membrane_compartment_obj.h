...
/* $Id: ntbp_membrane_compartment_obj.h,v 1.6 2003/06/20 16:15:05 face Exp $
...
public:
    ...
    const NTBP_membrane_current_o * Current(NTsize currentIndex) {
        NT_ASSERT((currentIndex > 0) && (currentIndex-1 < currentVec.size()));
    ...
    NTreturn Set_temperature(NTreal newTemp /* in Celsius */) {
        temperature = newTemp;
        for (NTsize i = 0; i < currentVec.size(); i++) currentVec[i]->Set_temperature(newTemp);
        return NT_SUCCESS;
    }
    /* in Celsius */
