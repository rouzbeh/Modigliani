/**\file ntbp_potassium_multi_current_obj.cpp - squid giant axon sodium conducta nce class implementation 
...
/* $Id: ntbp_potassium_multi_current_obj.cpp,v 1.1.1.1 2004/12/16 01:38:36 face Exp $
 * $Log: ntbp_potassium_multi_current_obj.cpp,v $
...
NT_gaussian_rnd_dist_o NTBP_potassium_multi_current_o::normalRnd;
bool NTBP_potassium_multi_current_o::initTableLookUp = false;
NTreal NTBP_potassium_multi_current_o::alphaNvec[15000];
NTreal NTBP_potassium_multi_current_o::betaNvec[15000];

/* ***      CONSTRUCTORS	***/
/** Create a NTBP_potassium_multi_current_o */
NTBP_potassium_multi_current_o::NTBP_potassium_multi_current_o(NTreal newArea,
		NTreal newDensity, NTreal newChannelConductance, NTreal newVBase,
		NTreal newTBase, NTreal newEK) :
	NTBP_multi_current_o(newEK, /* mV */
	newDensity, newArea, newChannelConductance, newVBase) {
	NT_ASSERT (newArea >= 0);
...
/* ***      COPY AND ASSIGNMENT	***/
NTBP_potassium_multi_current_o::NTBP_potassium_multi_current_o(
		const NTBP_potassium_multi_current_o & original)
:
...
const NTBP_potassium_multi_current_o&
NTBP_potassium_multi_current_o::operator=(
		const NTBP_potassium_multi_current_o & right) {
	if (this == &right)
...
inline void NTBP_potassium_multi_current_o::ComputeRateConstants() {
	NTreal vM = voltage;
	NTreal q10Factor = NTBP_TemperatureRateRelation(_temperature(), baseTemp,
			_q10());
...
/* ***  PROTECTED                         ***   */
/*NTreturn DEPRECATED
 NTBP_potassium_multi_current_o::SetDensityChild(NTreal channelDensity)// in channels/muMeter^2
 {
...
/** in kHz */
inline NTreal NTBP_potassium_multi_current_o::ComputeChannelStateTimeConstant() const {
	return channelsPtr->ComputeChannelStateTimeConstant(voltage);
}

/** No descriptions */
inline NTreal NTBP_potassium_multi_current_o::OpenChannels() const {
	...
	default:
		cerr
				<< "NTBP_potassium_multi_current_o::OpenChannels - ERROR : Unsupported simulation mode for OpenChannels()."
				<< endl;
		return 0;
	}
}

inline NTreal NTBP_potassium_multi_current_o::ComputeConductance() {
	switch (_simulationMode()) {
	case NTBP_GILLESPIE:
	case NTBP_SINGLECHANNEL:
	case NTBP_BINOMIALPOPULATION:
	...
	default:
		cerr
				<< "NTBP_potassium_multi_current_o::ComputeConductance - ERROR : Unsupported simulation mode for ComputeConductance."
				<< endl;
		return 0;
	}
}

inline NTreturn NTBP_potassium_multi_current_o::StepCurrent() {
	vector<NTreal> vec(2);
	vec[0] = alphaN;
	vec[1] = betaN;
	...
	case NTBP_LANGEVIN:
		...
		do {
			counter++;
			tmpN = _timeStep() * normalRnd.RndVal() * sqrt(
					(alphaN * (1 - n) + betaN * n) / _numChannels());
	...
	default:
		cerr
				<< "NTBP_potassium_multi_current_o::StepCurrent - ERROR : Unsupported simulation mode."
				<< endl;
		return NT_PARAM_UNSUPPORTED;
	}
}

void NTBP_potassium_multi_current_o::ShowParam() const {
