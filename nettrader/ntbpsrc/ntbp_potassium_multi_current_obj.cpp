...
/* ***      COPY AND ASSIGNMENT	***/
NTBP_potassium_multi_current_o::NTBP_potassium_multi_current_o(const NTBP_potassium_multi_current_o & original)
:
...
NTBP_potassium_multi_current_o::operator= (const NTBP_potassium_multi_current_o & right)
{
...
inline void
NTBP_potassium_multi_current_o::ComputeRateConstants(NTreal vM)
{
	NTreal q10Factor = NTBP_TemperatureRateRelation(_temperature(), baseTemp, _q10());
...
/** in kHz */
inline NTreal
NTBP_potassium_multi_current_o::ComputeChannelStateTimeConstant() const
{
...
/** No descriptions */
inline NTreal
NTBP_potassium_multi_current_o::OpenChannels() const
{
	switch(_simulationMode()) {
		...
		default:
				cerr << "NTBP_potassium_multi_current_o::OpenChannels - ERROR : Unsupported simulation mode for OpenChannels()." << endl;
				return 0;			}
}


inline NTreal
NTBP_potassium_multi_current_o::ComputeConductance()
{
	switch(_simulationMode()) {
		case NTBP_GILLESPIE:
		case NTBP_SINGLECHANNEL:
...
//					mean = uniformRnd.RndVal()
		...
		default:
				cerr << "NTBP_potassium_multi_current_o::ComputeConductance - ERROR : Unsupported simulation mode for ComputeConductance." << endl;
				return 0;
	}
}
...
inline NTreturn NTBP_potassium_multi_current_o::StepCurrent()
{
	...
	switch(_simulationMode()) {
		...
		case NTBP_DETERMINISTIC:
				...
				break;
		default:
				cerr << "NTBP_potassium_multi_current_o::StepCurrent - ERROR : Unsupported simulation mode." << endl;
