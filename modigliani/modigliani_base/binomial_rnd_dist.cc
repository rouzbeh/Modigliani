...
using modigliani_base::Binomial_rnd_dist;
...
Binomial_rnd_dist::Initializer::Initializer() {
  ...
  debug_print("%s\n", "This may take a while, populating LogGamma tables");
...
Binomial_rnd_dist::Binomial_rnd_dist(Real pp, Size num):Rnd_dist() {
  M_ASSERT((pp >= 0) && (pp <= 1));
  p = pp;
  M_ASSERT(num > 0);
  n = num;
...
inline Real Binomial_rnd_dist::RndVal() const {
  ...
  static modigliani_base::Size nsave = -1;
  static modigliani_base::Size ignbin, i, ix, ix1, k, m, mp, T1;
  ...
  S10:
  psave = p;
  pp = M_Min(static_cast<double>(psave), 1.0 - psave);
  q = 1.0 - pp;
  S20:xnp = n * pp;
  nsave = n;
  if (xnp < 30.0)
  ...
  fm = m;
  xnpq = xnp * q;
  p1 = static_cast<modigliani_base::Size>(2.195 * sqrt(xnpq) - 4.6 * q) + 0.5;
  xm = fm + 0.5;
  xl = xm - p1;
  xr = xm + p1;
  ...
  qn = pow(static_cast < double >(q), static_cast < double >(n));
  r = pp / q;
  g = r * (n + 1);
S150:ix = 0;
  f = qn;
...
for (j = 1; j <= new_n; j++)
  if (uniformMT() < new_p)
    ...
    do {
      ...
      em = floor(em);
      t = 1.2 * sq * (1.0 + y * y)
        * exp(oldg - LogGamma(em + 1.0) - LogGamma(en - em + 1.0) + em * plog
              + (en - em) * pclog);
    } while (uniformMT() > t);
    bnl = em;
  }
...
inline Real Binomial_rnd_dist::LogGamma(Real val) const {
  int rVal = static_cast<int>(val);
  if ((val == static_cast<Real>(rVal)) && (rVal < M_LOGGAMMATABLEMAX)) {
