...
Multidim_histogram::Multidim_histogram(Real low, Real high, Size bins,
        Size dims) :
        oCube(dims, bins) {
...
Size Multidim_histogram::BinValue(const std::vector<Real> & valueVec) {
    ...
    for (Size ld = 0; ld < dim; ld++) {
        coor = Size(numBins * (valueVec[ld] - min) / range);
        if (coor < numBins) {
            coorVec[ld] = coor;
        } else {
            outsideBinVec[ld] += 1;
