...
/* ***      CONSTRUCTORS    ***/
/** Create a Image */
Image::Image(Size newWidth, Size newHeight, Size newColorDepth,
...
/** @short
 ...
 \bug        unknown
 */
ReturnEnum Image::Load(std::string filename, ImageFileType fileType) {
    if (AUTO == fileType)
    ...
    if (fileType == UNKNOWN) {
        std::cerr
                ...
                << std::endl;
        return (ReturnEnum::WRONG_FILE_TYPE);
    }
...
/** @short     File type idendification is based on last four  characters in "filename"
 ...
 \bug        unknown
 */
modigliani_base::ImageFileType Image::Filename2Filetype(std::string filename) {
    Size length = filename.length();
...
ReturnEnum Image::LoadTGA(std::string filename) {
    ...
    if (!(s = fopen(filename.c_str(), "r+bt")))
        return (ReturnEnum::FILE_NOT_FOUND);
    ...
    for (Size ll = 0; ll < width * height * (int) dataFormat; ll++) {
        tmp1 = imageData[ll];
        tmp2 = (int) tmp1;
        imageVec[ll] = tmp2;
