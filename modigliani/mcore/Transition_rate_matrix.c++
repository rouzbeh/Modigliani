...
NTBP_transition_rate_matrix_o::NTBP_transition_rate_matrix_o(
		mbase::Msize new_num_states, mbase::Mreal new_min, mbase::Mreal new_max, mbase::Mreal new_step) :
		min(new_min), max(new_max), step(new_step), num_states(new_num_states) {
	int length = (floor((max - min) / step + 0.5) + 1)	* num_states * num_states;
	_probMatrices = new mbase::Mreal[length];
...
void NTBP_transition_rate_matrix_o::setTransitionProbability(mbase::Mreal voltage,
		mbase::Msize start, mbase::Msize stop, mbase::Mreal probability) {
...
mbase::Msize NTBP_transition_rate_matrix_o::get_index(mbase::Mreal voltage){
	return (floor((voltage - min) / step +0.5));
}

mbase::Mreal NTBP_transition_rate_matrix_o::getTransitionProbability(mbase::Mreal voltage,
		mbase::Msize start, mbase::Msize stop) {
	return (_probMatrices[get_index(voltage) * num_states * num_states
			+ (start - 1) * num_states + (stop - 1)]);
}

mbase::Mreal NTBP_transition_rate_matrix_o::getTransitionProbability(mbase::Msize index,
		mbase::Msize start, mbase::Msize stop) {
