/**\file ntbp_tridiag_solver_obj.cpp - Tridiag_solver class implementation
...
/* ***      CONSTRUCTORS	***/
/** Create a Tridiag_solver */
Tridiag_solver::Tridiag_solver(mbase::Size_t numNewCompartments) {
...
Tridiag_solver::Tridiag_solver(
		const Tridiag_solver __attribute__((unused)) & original) :
		mbase::Obj() {
...
/*
 mbase::Mreturn
 Tridiag_solver::Setup(std::vector <mbase::Real> newLVec, std::vector <mbase::Real> newUVec)
 {
 std::cerr << "OBSOLETE code called";
...
/** @short       
 ...
 \bug        unknown
 */
std::vector<mbase::Real> Tridiag_solver::Solve(std::vector<mbase::Real> dVec,
	...
	mbase::Size_t m = numCompartments;
	/* forward elimination */
	//dVec[0] = dVec[0];
...
/**  */
mbase::Mreturn Tridiag_solver::Resize(mbase::Size_t numNewCompartments) {
	M_ASSERT(numNewCompartments > 0);
	numCompartments = numNewCompartments;
	return (mbase::M_SUCCESS);
}
