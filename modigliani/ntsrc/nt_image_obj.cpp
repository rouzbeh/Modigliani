...
/* ***      CONSTRUCTORS	***/
/** Create a NT_image_o */
NT_image_o::NT_image_o(NTsize newWidth, NTsize newHeight, NTsize newColorDepth,
...
NT_image_o::NT_image_o(const NT_image_o & original) :
		NT_o(), width(original.width), height(original.height), colorDepth(
...
/** @short
 ...
 \bug        unknown
 */
NTreturn NT_image_o::Load(string filename, NTimageFileType fileType) {
	if (NT_AUTO == fileType)
		fileType = Filename2Filetype(filename);
	if (fileType == NT_UNKNOWN) {
		cerr
				<< "NT_image_o::Load - Error : In filename="
				<< filename
...
/** @short     File type idendification is based on last four  characters in "filename"
 ...
 \bug        unknown
 */
NTimageFileType NT_image_o::Filename2Filetype(string filename) {
	NTsize length = filename.length();

	if (filename.substr(length - 4, 1) != ".") {
		cerr
				<< "NTimageFileType Filename2Filetype - Warning : filename="
				<< filename
				<< " does "
...
NTreturn NT_image_o::LoadTGA(string filename) {
