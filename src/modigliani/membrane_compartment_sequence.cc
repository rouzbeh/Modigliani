...
modigliani::ReturnEnum Membrane_compartment_sequence::PushBack(
  Membrane_compartment *compartPtr) {
  ...
  if (compartment_vec_.size() !=
      num_compartments_) return modigliani::ReturnEnum::FAIL;

  return modigliani::ReturnEnum::SUCCESS;
}

modigliani::ReturnEnum Membrane_compartment_sequence::InjectCurrent(
  modigliani::Real current,
  modigliani::Size compartmentId) {
  if ((compartmentId < 1) ||
      (compartmentId >
       num_compartments())) return modigliani::ReturnEnum::PARAM_OUT_OF_RANGE;
...
modigliani::ReturnEnum Membrane_compartment_sequence::Step() {
  if (!initialised_) {
    std::cerr
    ...
    if (Init() != modigliani::ReturnEnum::SUCCESS) {
      std::cerr
        << "Membrane_compartment_sequence::Step() "
  ...
  if (use_gillespie_) {
    ...
    std::vector<modigliani::Real> compartmentTau_vec_(num_compartments());
    modigliani::Real val;
    modigliani::Real sum;
    modigliani::Real sequenceTau;
    bool integrateStep = false;
    modigliani::Real newDeltaT;
    modigliani::Real maxDeltaT;

    modigliani::Real tStar = 0.0;
    ...
    do {
      ...
      for (modigliani::Size ll = 0; ll < num_compartments(); ll++) {
      ...
      newDeltaT = log(1 / uni_(rng_)) / sequenceTau; // sequenceTau in [kHz]
                                                     // while newDeltaT in [ms]
      std::cerr << "NEW DELTA T=" << newDeltaT << std::endl;
      maxDeltaT = 1;                                 // maximumTimeStep ought to
                                                     // be 1 ms
  ...
  std::vector<modigliani::Real> tmpVVec;
  modigliani::Real omega = 0.0;
  modigliani::Size ll    = 0;

  /* load voltage std::vector and rhs-std::vector */
  if (num_compartments_ > 1) {
    for (ll = 0; ll < num_compartments_; ll++) {
      /* omega should have units of mV : mSec nA / muF = muV */
      omega = 1e-3 /* mV/muV */
              * (timestep() /
                 compartment_vec_[ll]->CompartmentMembraneCapacitance())
              * (compartment_vec_[ll]->CompartmentMembraneNetCurrent());
      r_vec_[ll] = compartment_vec_[ll]->vm() + omega; // compute RHS of finite
                                                       // difference equation
      // OMEGA is substracted because the currents are of the opposite sign to
      // Aldo's PhD thesis
    }

    std::vector<modigliani::Real> vVec = NumericalRecipesSolveTriDiag(l_vec_,
                                                                           d_vec_,
                                                                           u_vec_,
                                                                           r_vec_);
    ...
    /* set new voltage */
    for (ll = 0; ll < num_compartments_; ll++) {
      compartment_vec_[ll]->Step(vVec[ll]); // Step also advances the voltage ->
                                            // ignore by using vVec
    }
  }
  else compartment_vec_[ll]->Step();
  return modigliani::ReturnEnum::SUCCESS;
}

modigliani::ReturnEnum Membrane_compartment_sequence::Init() {
  ...
  if (num_compartments_ > 1) {
    ...
    for (ll = 1; ll < num_compartments_ - 1; ll++) {
      ...
      auto sigma_backward =
        _sigma(compartment_vec_[ll - 1], compartment_vec_[ll]);
    ...
    l_vec_[num_compartments_ - 1] = -2.0 * sigma_backward; // vonNeumann
                                                           // boundary
                                                           // conditions
  }
...
modigliani::Real Membrane_compartment_sequence::_sigma(
  const Membrane_compartment *from,
  const Membrane_compartment *to) const {
  modigliani::Real deltaT = timestep();

  // sigma is unit-free  (scaled by 0.1) [sigma] = mSec muM / (muM^2 Ohm cm) *
  // (muM^2 / muF)
  modigliani::Real output = 0.1
                                 * (deltaT / to->CompartmentMembraneCapacitance())
                                 / (to->ra()
                                    * (2 * to->length() /
                                       (4 * 3.1415 * pow(to->radius(), 2)))
                                    + from->ra()
                                    * (2 * from->length() /
                                       (4 * 3.1415 * pow(from->radius(), 2))));

  return output;
}

modigliani::ReturnEnum Membrane_compartment_sequence::InitialStep() {
...
modigliani::Real Membrane_compartment_sequence::
CompartmentSequenceChannelStateTimeConstant() const {
  std::cerr
    <<
...
std::vector<modigliani::Real>Membrane_compartment_sequence::
NumericalRecipesSolveTriDiag(
  const std::vector<modigliani::Real>& lNewVec,
  const std::vector<modigliani::Real>& dNewVec,
  const std::vector<modigliani::Real>& uNewVec,
  const std::vector<modigliani::Real>& rNewVec) const {
  modigliani::Size n = lNewVec.size();
  std::vector<modigliani::Real> vNewVec(n);
  ...
  modigliani::Real bet;
  std::vector<modigliani::Real> gam(n);
