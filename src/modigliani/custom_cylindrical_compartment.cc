...
#include "modigliani/custom_cylindrical_compartment.h"

using modigliani::Real;

namespace modigliani {
  Custom_cylindrical_compartment::
  Custom_cylindrical_compartment(const modigliani::Real
                                 newLength,
                                 const
                                 modigliani::Real
                                 newDiameter,
  ...
  modigliani::ReturnEnum Custom_cylindrical_compartment::
  ...
  modigliani::ReturnEnum Custom_cylindrical_compartment::
    Step(const modigliani::Real newVM) {
    set_vm(newVM);
    // for every current
    for (modigliani::Size it = 0; it < NumberCurrents(); it++) {
      ...
      if ((custom_current_vec_[it]).track) {
        ...
        modigliani::Real ions_picomoles = current->current() * timestep()
          / 96485.3415;
  ...
  Custom_cylindrical_compartment::SetupOutput(std::string output_file_name) {
    ...
    if (file_length > 0) {
      ...
      modigliani::Size number_of_columns = 4 * NumberCurrents() + 1;
  ...
  SetInsideConcentration(modigliani::Size currentIndex,
  ...
  void Custom_cylindrical_compartment::
      SetOutsideConcentration(modigliani::Size currentIndex,
                              Real new_concentration) {
    assert((currentIndex > 0) && (currentIndex - 1 < NumberCurrents()));
