...
#include "modigliani/membrane_patch.h"

namespace modigliani {
  Membrane_patch::Membrane_patch(modigliani::Real newArea,
                                 modigliani::Real newCM)
  :Membrane_compartment(newArea, 6.3, newCM, 0) {
  } Membrane_patch::~Membrane_patch() {
  }

  inline modigliani::ReturnEnum Membrane_patch::Step() {
    /*       [mV] == [10^-3 ms nA /muF] */
    modigliani::Real deltaV = 1e-3 * timestep()
  ...
  modigliani::ReturnEnum Membrane_patch::InitialStep() {
