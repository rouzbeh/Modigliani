 ...
 */
  ...
  public:
     ...
     */
    Custom_cylindrical_compartment(const modigliani::Real length,
                                   const modigliani::Real diameter,
                                   const modigliani::Real cM,
                                   const modigliani::Real rA,
                                   const modigliani::Real newTemperature,
                                   const modigliani::Real vouvinratio = 8);
    ...
    virtual modigliani::ReturnEnum Step
      (const modigliani::Real newVM) override;
     ...
     * @warning Call after having attached all currents.
     */
    virtual modigliani::ReturnEnum WriteOutput() const override;
     ...
     */
    Real InsideConcentration(modigliani::Size currentIndex) const {
     ...
     * @warning Current indexed [1..m]
     */
    Real OutsideConcentration(modigliani::Size currentIndex) const {
      assert((currentIndex > 0) && (currentIndex - 1 < NumberCurrents()));
     ...
     * @warning Current indexed [1..m]
     */
    Real ReversalPotential(modigliani::Size currentIndex) const {
      assert((currentIndex > 0) && (currentIndex - 1 < NumberCurrents()));
    ...
    virtual modigliani::ReturnEnum set_temperature
     (modigliani::Real newTemp) override {
      Membrane_compartment::set_temperature(newTemp);
      nernst_multiplier = 1000 * modigliani::R *
        (newTemp + modigliani::ZERO_CELSIUS) / modigliani::F;
     ...
     * @warning Current indexed [1..m]
     */ void SetInsideConcentration(modigliani::Size currentIndex,
     ...
     * @warning Current indexed [1..m]
     */
    void SetOutsideConcentration(modigliani::Size currentIndex,
                                 Real new_concentration);

     private: std::vector < custom_current > custom_current_vec_;
    const modigliani::Real volumeratio;
     modigliani::Real nernst_multiplier;
  };
}  // namespace modigliani
