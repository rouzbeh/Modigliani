...
#include "modigliani/lua_based_stochastic_voltage_gated_channel.h"
...
namespace modigliani {
  bool Lua_based_stochastic_voltage_gated_channel::initTableLookUp = false;
   ...
   std::map < string,
    double > Lua_based_stochastic_voltage_gated_channel::base_temperature_map;
   std::map < string,
   ...
   Lua_based_stochastic_voltage_gated_channel::
    Lua_based_stochastic_voltage_gated_channel(modigliani::Real newArea,
  ...
  void Lua_based_stochastic_voltage_gated_channel::load_file(
      string fileName,
      double temperature,
      double time_step) {
    ...
    modigliani::Real length = floor((maxV - minV) / step + 0.5) + 1;
    for (modigliani::Size i = 1; i <= number_of_states_map[fileName]; ++i) {
      for (modigliani::Size j = 1; j <= number_of_states_map[fileName];
           ++j) {
        for (modigliani::Size k = 0; k < length; k++) {
          modigliani::Real voltage = minV + step * k;
          ...
          lua_pop(L, 1);
          double probability =
            TemperatureRateRelation(temperature, base_temperature_map[fileName],
                                    prob_q10)
  ...
  inline modigliani::ReturnEnum
    ...
    switch (simulation_mode()) {
      ...
      default:
        std::cerr
          << "Lua_based_stochastic_voltage_gated_channel::StepCurrent"
          << " - ERROR : Unsupported simulation mode." << std::endl;
  ...
  inline modigliani::
    Real Lua_based_stochastic_voltage_gated_channel::OpenChannels() const {
    return (channels_ptr_->NumOpen());
  } inline modigliani::
    Real Lua_based_stochastic_voltage_gated_channel::ComputeConductance() {
    return (set_conductance(channels_ptr_->NumOpen() * conductivity_));
  }

  inline modigliani::
    Real Lua_based_stochastic_voltage_gated_channel::ComputeTimeConstant() const
  {
    return (channels_ptr_->ComputeChannelStateTimeConstant(voltage_));
  } modigliani::Real Lua_based_stochastic_voltage_gated_channel::
    lua_get_real(lua_State * L, string name) {
    lua_getglobal(L, name.c_str());
    modigliani::Real ret = lua_tonumber(L, -1);
    lua_pop(L, 1);
    return (ret);
  }
}  // namespace modigliani
